...
if (isset($_REQUEST["q"])) {
    ...
    if (!empty($search_term)) {
        $condition[] = " `es_name` LIKE '%{$search_term}%'";
    }
}

if (!empty($id)) {
    $condition[] = " `es_ev_id` = '$id'";
}

$condition[] = "`es_status`='Active' AND `es_ev_id` != '1'";
...
if (empty($error)) {
    ...
    $GET_SQL = "SELECT `es_id` as id ,`es_name` as text FROM `event_sessions` 
       ";
    if (!empty($where)) {
        $GET_SQL .= $where;
    }
    $GET_SQL .= " GROUP BY `es_id` ";
